...
<div className="max-w-6xl mx-auto bg-gray-900 rounded-2xl shadow-md p-6 mb-10">
 <h2 className="text-xl font-semibold text-green-400 mb-4">
  Completed Courses ({userData?.completedCourses?.length || 0})
</h2>
  ...
  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
    {userData?.completedCourses?.length > 0 ? (
      userData.completedCourses.map((course, index) => (
        <div
          key={index}
          className="bg-gray-700 p-4 rounded-lg border border-gray-600 shadow hover:shadow-lg transition text-white"
        >
          <h3 className="text-lg font-semibold mb-2">{course.title}</h3>
          <p className="text-sm text-gray-300 mb-1">Course ID: {course._id}</p>
          <p className="text-sm text-gray-400">Completed on: {new Date(course.completedAt).toLocaleDateString()}</p>
        </div>
      ))
    ) : (
  ...
  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
    {userData?.certificates?.length > 0 ? (
      userData.certificates.map((cert, i) => (
        ...
        >
          <h3 className="text-lg font-semibold text-white mb-2">{cert.courseTitle}</h3>
          <p className="text-sm text-gray-300 mb-3">
            Certificate ID: <span className="text-white">{cert._id}</span>
          </p>
      ...
      <p className="text-gray-400">No certificates issued.</p>
    )}
  </div>
</div>
