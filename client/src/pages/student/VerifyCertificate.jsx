...
const VerifyCertificate = () => {
  ...
  // ✅ Generate QR code only after certificateData is set and canvas is mounted
  useEffect(() => {
    const generateQR = async () => {
      if (canvasRef.current && certificateData?.certificateId) {
        try {
          await QRCode.toCanvas(canvasRef.current, certificateData.certificateId, { width: 90 });
        } catch (err) {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center justify-start bg-gray-50 px-4 pt-10 pb-20">
      ...
      {message && (
        <div
          className={`text-xl mt-2 px-6 py-3 rounded ${messageType === 'error'
