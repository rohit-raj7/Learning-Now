...
const App = () => {
  ...
  const { educator, user } = useContext(AppContext); // ✅ Use context

  return (
    <div className="text-default min-h-screen bg-gray-800">
      <ToastContainer />

      {/* ✅ Show EducatorNavbar only if logged in as educator */}
      {!isEducatorRoute && educator && user && (
        <EducatorNavbar user={user} bgColor="bg-[#112d46]" />
      )}

      {/* ✅ Else show Student Navbar */}
      {!isEducatorRoute && (!educator || !user) && <Navbar />}
